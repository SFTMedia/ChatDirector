...
public class FancyMessageSerializer_v0_2_0 extends JsonSerializer<FancyMessage_v0_2_0> {

    @Override
    public void serialize(FancyMessage_v0_2_0 value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        boolean root = false;
        if ((value.getText() == null || value.getText().equals("")) && value.getClick().equals(FancyMessageEnum.NONE)
            ...
            if (!value.getNext().isEmpty()) {
                FancyMessage_v0_2_0 tempRoot = new FancyMessage_v0_2_0();
        ...
        if (!value.getNext().isEmpty() && root) {
            for (FancyMessage_v0_2_0 fancyMessage : value.getNext()) {
                gen.writeStartObject();
